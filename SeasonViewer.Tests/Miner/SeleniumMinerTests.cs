namespace SeasonViewer.Tests.Miner
{
    using Microsoft.Extensions.Configuration;
    using NUnit.Framework;
    using SeasonViewer.Core;
    using SeasonViewer.Core.Miner;
    using SeasonViewer.Infrastructure.Miner;
    using System;
    ...
    public class SeleniumMinerTests : TestBase
    {
        protected string GetResource(params string[] path)
        {
            ...
            var stream = System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceStream(name)
                ?? throw new Exception($"Could find the following resource: {name}.");
        ...
        public void TestParseAnimeFromJson()
        {
            ...
            Assert.That(actualAnimes.Count, Is.EqualTo(expectedAnimes.Length));

            for (var i = 0; i < expectedAnimes.Length; i++)
            {
                var expected = expectedAnimes[i];
                var actual = actualAnimes.ElementAt(i);
        ...
        public void TestParseDuckDuckGoSearchFromJson()
        {
            ...
            Assert.That(searchResult.Count, Is.EqualTo(expectedResults.Length));

            for (var i = 0; i < expectedResults.Length; i++)
            {
                var expected = expectedResults[i];
                var actual = searchResult.ElementAt(i);
        ...
        public void TestParseMalListFromJson()
        {
            ...
            Assert.That(expectedResults, Is.Not.Empty, "unit test implementation");
            Assert.That(listEntries.Count, Is.EqualTo(expectedResults.Length));

            for (var i = 0; i < expectedResults.Length; i++)
            {
                var expected = expectedResults[i];
                var actual = listEntries.ElementAt(i);
        ...
        public void TestParseAnime()
        {
            ...
            var animes = testee.MineSeasonAnimeAsync(season);
